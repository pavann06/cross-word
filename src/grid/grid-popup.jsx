...
function PopupGrid({ onSubmit, onClose,  existingGrid }) {
  const [gridLetters, setGridLetters] = useState(existingGrid); // Initialize grid with existing letters
  const [errorMessage, setErrorMessage] = useState("");

  useEffect(() => {
    setGridLetters(existingGrid); // Update grid when existing grid changes
  }, [existingGrid]);
  ...
  const handleInputChange = (rowIndex, columnIndex, value) => {
    ...
    if (existingGrid[rowIndex][columnIndex]) {
      // Handle error here, maybe show a message or prevent further input
      setErrorMessage("A letter already exists in this position.");
      return;
    }
